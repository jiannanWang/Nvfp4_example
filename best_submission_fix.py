...
class Sm100BlockScaledDenseGemmKernel:
    ...
    def _compute_grid(
        ...
        # Compute raw grid dimensions
        grid_m = cute.ceil_div(c.layout.shape[0], cta_tile_shape_mnk[0])
        grid_n = cute.ceil_div(c.layout.shape[1], cta_tile_shape_mnk[1])
        grid_l = c.layout.shape[2]
        
        # Round up grid dimensions to be divisible by cluster shape
        # This is required by CUDA - grid dims must be evenly divisible by cluster dims
        grid_m = cute.ceil_div(grid_m, cluster_shape_mn[0]) * cluster_shape_mn[0]
        grid_n = cute.ceil_div(grid_n, cluster_shape_mn[1]) * cluster_shape_mn[1]
        
        return (grid_m, grid_n, grid_l)
